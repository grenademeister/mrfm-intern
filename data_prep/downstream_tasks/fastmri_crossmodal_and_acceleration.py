...
# crossmodal pairs (a, b): generate a->b crossmodal and a-acceleration
MODALITY_PAIRS = [("t1", "t2")]

# output directories
OUTPUT_DIRS_CROSS = {
    (src, tgt): f"{OUTPUT_BASE_DIR}/fastmri_crossmodal_mat_{src}to{tgt}"
    for src, tgt in MODALITY_PAIRS
}
OUTPUT_DIRS_ACCEL = {
    src: f"{OUTPUT_BASE_DIR}/fastmri_acceleration_mat_{src}"
    for src, _ in MODALITY_PAIRS
}
...
def get_fastmri_case(case_id: str, root: str | Path) -> dict[str, Path]:
    ...
    for p in modality_root.glob(pattern):
        ...
        if "T1" in n and ("POST" in n or "CONTRAST" in n):
            ...
            modality = "flair"
        elif "T2" in n and ("POST" in n or "CONTRAST" in n):
            continue
        elif "T2" in n:
            modality = "t2"
        elif "T1" in n:
...
def choose_slice_indices(depth: int, num_slices: int, rng: random.Random) -> list[int]:
    max_idx = int(depth * SLICE_NUM_RATIO)
    if max_idx <= 0 or num_slices > max_idx:
        return []
    slices = rng.sample(range(max_idx), k=num_slices)
...
def _init_worker(
    ...
    global _WORKER_UIDS, _WORKER_OUT_DIRS_CROSS, _WORKER_OUT_DIRS_ACCEL, _WORKER_SEED
    _WORKER_UIDS = uids
...
def _generate_sample(idx: int) -> None:
    ...
    for src, tgt in MODALITY_PAIRS:
        if src not in case_files or tgt not in case_files:
            continue

        # slice selection based on fastmri style using src volume
        try:
        ...
        slice_indices = choose_slice_indices(src_volume.shape[0], SLICES_PER_VOLUME, rng)
        if not slice_indices:
            continue

        # acceleration uses src only
        try:
        ...
        for slice_idx in slice_indices:
            try:
                ...
                cross_path = cross_out_dir / f"fastmri_{case_id}_{src}_to_{tgt}_s{slice_idx + 1:03d}_n{idx + 1:05d}.mat"
                savemat(cross_path, cross_data)

                # acceleration: undersampled src -> src
                accel_instruction = make_instruction_accel(src, pf, rng)
                accel_data = {
                    "image": accel_image[slice_idx],
                    "label": src_volume[slice_idx],
                    "instruction": np.array(accel_instruction, dtype=object),
                    "text": np.array(src_header_text, dtype=object),
                    "image_header": np.array(src_header_text, dtype=object),
                }
                accel_out_dir = _WORKER_OUT_DIRS_ACCEL[src]
                accel_path = accel_out_dir / f"fastmri_{case_id}_{src}_r{pf}_s{slice_idx + 1:03d}_n{idx + 1:05d}.mat"
...
def main() -> None:
    ...
    with Pool(
        ...
        initargs=(uids, OUTPUT_DIRS_CROSS, OUTPUT_DIRS_ACCEL, SEED),
    ) as pool:
    ...
    for src, out_dir in OUTPUT_DIRS_ACCEL.items():
        count = len(list(Path(out_dir).glob("*.mat")))
        print(f"Saved {count} acceleration samples for {src} to {out_dir}")
