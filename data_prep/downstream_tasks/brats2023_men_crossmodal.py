...
BRATS_DIR = f"{DATA_DIR}/brats2023_men"
SEED = 42
...
if MODALITY_PAIRS is None:
    ...
    OUTPUT_DIRS = {
        (src, tgt): f"{OUTPUT_BASE_DIR}/brats2023_men_crossmodal_mat_{src}to{tgt}"
...
def find_all_uids(root: str | Path) -> list[str]:
    root = Path(root)
    pat = re.compile(r"BraTS-MEN-(\d+)-000-") # only consider the first scan
    uids: set[str] = set()
...
def get_brats_case(case_id: str, root: str | Path) -> dict[str, Path]:
    ...
    files: dict[str, Path] = {}
    seg_pattern = f"BraTS-MEN-{case_id}-000-seg.nii.gz"
    ...
    for m in VALID_MODALITIES:
        pattern = f"BraTS-MEN-{case_id}-000-{m}.nii.gz"
    ...
    for src, tgt in pairs_to_process:
        ...
        for slice_idx in slice_indices:
            try:
                ...
                out_path = out_dir / f"brats2023_men_{case_id}_{src}_to_{tgt}_s{slice_idx + 1:03d}_n{idx + 1:05d}.mat"
