...
class VisionTextDecoder(nn.Module):
    def __init__(
        ...
        self.model_dim = min(decoder_feature_chans * 16, 256)
        if self.model_dim % 8 != 0:
            self.model_dim = max(128, decoder_feature_chans * 8)

        self.num_heads = max(2, self.model_dim // 64)
        self.depth = 2
        self.mlp_ratio = 2.0
        self.unet_dim = max(64, self.model_dim // 2)
        ...
        self.stack_proj = nn.ModuleList()
        self.xt_proj_layers = nn.ModuleList()
        self.unet_blocks = nn.ModuleList()
        for i in range(num_pool_layers):
            in_ch = feature_chans * (2**i)
            self.stack_proj.append(
                nn.Sequential(
                    nn.Conv2d(in_ch, self.unet_dim, kernel_size=1, padding=0),
                    nn.GroupNorm(4, self.unet_dim),
                    nn.SiLU(inplace=True),
                )
            )
            self.xt_proj_layers.append(
                nn.Sequential(
                    nn.Conv2d(self.input_chans, self.unet_dim, kernel_size=1, padding=0),
                    nn.GroupNorm(4, self.unet_dim),
                    nn.SiLU(inplace=True),
                )
            )
            self.unet_blocks.append(
                nn.Sequential(
                    nn.Conv2d(self.unet_dim * 2, self.unet_dim, kernel_size=3, padding=1),
                    nn.GroupNorm(4, self.unet_dim),
                    nn.SiLU(inplace=True),
                )
            )
        self.stack_residual_proj = nn.Sequential(
            nn.Conv2d(self.unet_dim, self.model_dim, kernel_size=1, padding=0),
            nn.GroupNorm(4, self.model_dim),
            nn.SiLU(inplace=True),
        )

        self.blocks = nn.ModuleList([DiTBlock(self.model_dim, self.num_heads, self.mlp_ratio) for _ in range(self.depth)])
        ...
        self.refine = nn.Sequential(
            nn.Conv2d(self.out_chans, self.out_chans, kernel_size=3, padding=1),
            nn.SiLU(inplace=True),
            nn.Conv2d(self.out_chans, self.out_chans, kernel_size=3, padding=1),
        )
    ...
    def _prepare_stack_residual(
        self,
        stack_feat: list[torch.Tensor] | None,
        ...
        if not stack_feat:
            return None
        current = None
        for idx in range(len(stack_feat) - 1, -1, -1):
            feat = stack_feat[idx]
            proj = self.stack_proj[idx](feat)
            xt_scaled = F.interpolate(flow_xt, size=feat.shape[-2:], mode="bilinear", align_corners=False)
            xt_proj = self.xt_proj_layers[idx](xt_scaled)
            fuse = proj + xt_proj
            if current is None:
                current = fuse
            else:
                current = F.interpolate(current, size=feat.shape[-2:], mode="bilinear", align_corners=False)
                current = torch.cat([current, fuse], dim=1)
                current = self.unet_blocks[idx](current)
            if current.shape[-1] == target_hw:
                break
        if current is None:
            return None
        if current.shape[-1] != target_hw:
            current = F.interpolate(current, size=(target_hw, target_hw), mode="bilinear", align_corners=False)
        return self.stack_residual_proj(current)

    def forward(
        self,
        x: torch.Tensor,  # [B, 1025, 512]
        stack_feat: list[torch.Tensor],  # [B, 64, 512, 512], [B, 128, 256, 256], [B, 256, 128, 128], [B, 512, 64, 64]
        instruction: torch.Tensor = None,  # [B, 1536, 512]
        flow_xt: torch.Tensor | None = None,  # [B, 1, 512, 512]
        flow_t: torch.Tensor | None = None,  # [B, 1]
    ) -> torch.Tensor:
        if x.dim() != 3:
        ...
        instruction = self._prepare_instruction(instruction)  # [B, 512]

        flow_xt = F.interpolate(flow_xt, size=(self.image_width, self.image_width), mode="bilinear", align_corners=False)  # [B, 1, 512, 512]
        xt_tokens = self.patch_embed(flow_xt).flatten(2).transpose(1, 2)  # [B, 1024, model_dim]
        xt_tokens = xt_tokens + self.pos_embed
        stack_residual = self._prepare_stack_residual(
            stack_feat,
            flow_xt,
            self.image_width // self.patch_size,
        )  # [B, model_dim, 32, 32]
        if stack_residual is not None:
            stack_tokens = stack_residual.flatten(2).transpose(1, 2)  # [B, 1024, model_dim]
            xt_tokens = xt_tokens + stack_tokens  # [B, 1024, model_dim]

        flow_t = self._prepare_flow_t(flow_t, batch_size)
        cond_vec = self.time_mlp(flow_t)  # [B, model_dim]
        ...
        cond_tokens = self._pad_tokens(x)  # [B, 1025, token_dim]
        cond_tokens = self.cond_proj(cond_tokens)  # [B, 1025, model_dim]
        cond_vec = cond_vec + cond_tokens.mean(dim=1)  # [B, model_dim]

        for block in self.blocks:
            xt_tokens = block(xt_tokens, cond_tokens, cond_vec)

        xt_tokens = self.norm_out(xt_tokens)  # [B, 1024, model_dim]
        out_tokens = self.head(xt_tokens)  # [B, 1024, out_chans*patch^2]

        h = self.image_width // self.patch_size  # 32
        w = h  # 32
